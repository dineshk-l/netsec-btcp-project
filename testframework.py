...
NETEM_ALL = "{} {} {} {}".format(
    NETEM_CORRUPT, NETEM_DUP, NETEM_LOSS, NETEM_REORDER)
...
def run_command_with_output(command, input=None, cwd=None, shell=True, timeout=None, termination_func=None):
    ...
    try:
        process = subprocess.Popen(
            command, cwd=cwd, shell=shell, stdout=subprocess.PIPE, stdin=subprocess.PIPE)
        ...
        while True:
            try:
                ...
                [stdoutdata, stderrdata] = process.communicate(input, timeout)
...
class TestbTCPFramework(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._server_thread = threading.Thread(target=run_command_with_output,
                                               args=("python3 server_app.py -w {} -t {} -o {} -l {}".format(
                                                   WINSIZE, TIMEOUT, OUTPUTFILE, LOGLEVEL),),
                                               kwargs={"timeout": 20,
